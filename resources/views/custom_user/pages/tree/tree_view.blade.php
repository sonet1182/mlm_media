...
@section('content')
    <div class="body genealogy-body genealogy-scroll">
        ...
        <div class="d-flex gap-3 align-items-center">
            <form id="searchForm" action="{{ route('user.placement_tree_ajax') }}" method="post" class="d-flex">
                @csrf
                <input type="text" class="form-control form-control-sm" placeholder="Input User Id..." name="user_id"
                    style="width: 200px" />
                <button type="submit" class="btn btn-success btn-sm ms-2 mx-2">
                    <i class="fas fa-search"></i> Search
                </button>
            </form>
            <button id="reloadButton" class="btn btn-secondary btn-sm ms-2">
            ...
            fetch(url, {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        // Inject the new tree HTML into the page
                        document.getElementById('tree_box').innerHTML = data.html;
                    } else {
                        // Show error message
                        alert(data.message);
                    }
