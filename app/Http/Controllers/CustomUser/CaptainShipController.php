...
class CaptainShipController extends Controller
{
    function captain_ship()
    {
        $my_user_id = Auth::guard('customUser')->user()->id;
        $captainships = CustomUser::where('referrer_id', $my_user_id)->get();

        return view('custom_user.pages.captainship.list', compact('captainships'));
    }



    function captain_ship_points($id, $month = null, $year = null)
    {
        ...
        return view(
            'custom_user.pages.captainship.points',
            compact(
                'user',
